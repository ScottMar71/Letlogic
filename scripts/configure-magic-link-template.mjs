#!/usr/bin/env node
/**
 * Patch hosted Supabase Magic Link email template for SSR (token_hash flow).
 *
 ...
 *
 * Usage:
 *   SUPABASE_ACCESS_TOKEN=... npm run configure:magic-link-template
 */
...
const res = await fetch(
  `https://api.supabase.com/v1/projects/${projectRef}/config/auth`,
  {
    ...
    body: JSON.stringify({
      mailer_subjects_magic_link: "Your LetLogic sign-in link",
...
console.log(
  `Updated Magic Link email template for project ${projectRef}. Request a fresh sign-in email to test.`,
);
