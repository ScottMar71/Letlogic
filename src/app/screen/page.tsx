...
type PageProps = { searchParams: Promise<{ from?: string; intake?: string }> };

export default async function ScreenPage({ searchParams }: PageProps) {
  const { from, intake } = await searchParams;
  const user = await getAuthenticatedUser();
  if (!user) redirect(`/login?next=/screen`);

  const admin = createAdminClient();
  const [balance, counts, reanalyseFrom, intakeLookup] = await Promise.all([
  ...
  return (
    <AuthenticatedPage creditBalance={balance} width="wide" mainClassName="space-y-6">
      ...
      <ScreeningWorkspace
        key={reanalyseFrom?.applicationId ?? intakeFrom?.intakeLinkId ?? "new"}
        propertyId={reanalyseFrom?.propertyId ?? intakeFrom?.propertyId ?? undefined}
